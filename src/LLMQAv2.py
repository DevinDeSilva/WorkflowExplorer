...
class PlanningAgent:
    def __init__(
        ...
        Always start the plan with either identifying relevant entities (must use question entity and identify object, if so no atomic question is used) or retrieving objects 
        ...
        You must further identify all the entities relevant for sub questions this maybe classes, KG relations , grounding entities. next steps would be the traversal of the knowledge graph. 
        ...
        - Entities-Question: []
        - Classes: [provone:Program, eo:AITask]
        ...
        #### Step 3: 
        - Sub-question: Use the attribute information such as ID to determine the order of execution of the programs.
        - Atomic Question Used: Null
        - Entities-Question: []
        - Classes: [provone:Program]
        ...
        ### Step 2:
        - Sub question: What is the program associated with the execution with id 1_2?
        - Atomic Question Used: "What program was planned for this execution?"
        - Entities-Question: []
        - Classes: [provone:Execution, provone:Program]
        
        #### Step 3: 
        - Sub-question: Filter execution associated with system prompt generation function.
        - Atomic Question Used: Null
        - Entities-Question: []
        - Classes: [provone:Execution, provone:Program]
        ...
        #### Step 5: 
        - Sub-question: What are the attributes of the execution objects?.
        - Atomic Question Used: "Explores Attributes of a given object in the RDF graph."
        - Entities-Question: []
        - Classes: [provone:Execution, provone:Program]
        """
...
class SPARQLExecutorAgent:
    ...
    def link_object_in_kg(self, object_desc: str) -> List[str]:
    ...
    def run_step(self, main_question: str, step_name: str, prev_results: List[Dict[str, Any]]) -> Any:
        ...
        if atomic_question:
            ...
            _results = _results.to_dict(orient="records")
            self.logs[step_name]["sparql_results"] = _results
            
        elif entity_linking and len(entity_linking) > 0:
            linked_entities = []
            for entity_desc in entity_linking:
                linked_entity_ids = self.link_object_in_kg(entity_desc)
    ...
    def summarize_results(self, main_question: str, results: List[Dict[str, Any]]) -> str:
        ...
        Answer the question asked in the main question, be verbose but truthful to the information provided in the context.
        also provide a list of important facts that mainly effected the explanation (A list of URIs of the facts that mainly 
        effected the explanation)
        """
...
class SPARQLGeneratorAgent:
    ...
    def kbest_sparql_queries(self, question: str, k: int =3) -> Optional[str]:
        ...
        search_results = semantic_search(
            ...
            corpus_embeddings=self.vector_store['vec_repr'],
            top_k=k,
        )
        top_result_idx = []
        for idx in search_results[0]:
...
def question_asker_and_loger(
    ...
    common_utils.serialization.save_json(log_data, log_file_path)
...
if __name__ == "__main__":
    ...
    log_file_path = os.path.join(V2_DIR, f"logs/{question}.json")
